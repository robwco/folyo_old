class DesignerMailer < ActionMailer::Base
  ...
  def accepted_mail(designer)
    ...
    mail :subject => subject, :from => "Folyo <hello@folyo.me>", :to => designer.user.email
  end
  ...
  def rejected_reply(designer, job_offer)
   ...
   def client_message(message)
    ...
    mail :subject => subject, :from => "#{from.full_name} <#{from.email}>", :to => message.designer.user.email
  end
