class ClientMailer < ActionMailer::Base
  ...
  def new_job_offer(job_offer)
    ...
    @job_offer = job_offer
    mail :subject => subject, :from => "#{job_offer.client.user.full_name} <#{job_offer.client_email}>", :to => "hello@folyo.me"
  end
  ...
  def job_offer_replied(designer_reply)
    subject = "#{designer_reply.user.full_name} has replied to your offer"
    @designer_reply = designer_reply
    mail :subject => subject, :from => "#{designer_reply.user.full_name} <#{designer_reply.user.email}>", :to => designer_reply.job_offer.client_email
  end
end
